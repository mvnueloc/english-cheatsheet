...
function MotionCarousel(props: PropType) {
  ...
  return (
    <div
      className={cn(
        "w-full space-y-4 h-full [--slide-spacing:1.5rem] [--slide-size:55%]",
        className,
      )}>
      <div
        className={cn(
          "overflow-hidden h-full",
          controlsPosition === "overlay" && "relative",
        )}
        ref={emblaRef}>
          ...
          {slides.map((index) => {
            ...
            return (
              <motion.div
                key={index}
                className={cn(
                  "h-full mr-[var(--slide-spacing)] basis-[var(--slide-size)] flex-none flex min-w-0",
                  slideClassName,
                )}>
                <motion.div
                  className={cn(
                    "size-full flex items-center justify-center text-3xl md:text-5xl font-semibold select-none border-4 rounded-xl",
                    slideInnerClassName,
                  )}
                  initial={false}
                  animate={{
                  ...
                  {renderSlide ? renderSlide(index, isActive) : index + 1}
                </motion.div>
              </motion.div>
            );
          })}
        ...
        {showControls && controlsPosition === "overlay" ? (
          <div
            ...
            {showDots ? (
              <div
                className={cn(
                  "pointer-events-auto flex flex-wrap justify-end items-center gap-2",
                  dotsClassName,
                )}>
      ...
      {showControls && controlsPosition === "below" ? (
